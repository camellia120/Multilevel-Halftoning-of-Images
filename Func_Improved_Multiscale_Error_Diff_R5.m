...
%%%To decide whether we shoulde inverse the matrix.
     if i==5;
         if E{i-1}(s_exact,t_exact)/Bm_Mark_M{i-1}(s_exact,t_exact)>0.5; 
             Inverse=1;
         end
     end
...
Em(s_exact,t_exact)=0;
else
Bm(s_exact,t_exact)=0;
Bm_Mark(s_exact,t_exact)=1;
...
while(su==0);
     for p=1:2*d+1;
         for q=1:2*d+1;
             if (s_exact+p-d>=1)&&(s_exact+p-d<=N)&&(t_exact+q-d>=1)&&(t_exact+q-d<=N);
                 if Bm_Mark(s_exact+p-d,t_exact+q-d)==1;
                    H(p,q)=0;
                 else
...
%We just process the pixels in the Em matrix.
        if (s_exact+mm-d>=1)&&(s_exact+mm-d<=N)&&(t_exact+nn-d>=1)&&(t_exact+nn-d<=N);
              Em(s_exact+mm-d,t_exact+nn-d)=Em(s_exact+mm-d,t_exact+nn-d)-e*H1(mm,nn);
        end
    end
end  
Bm_Mark(s_exact,t_exact)=1;
