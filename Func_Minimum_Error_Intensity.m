function [s,t]=Func_Minimum_Error_Intensity(Input_Image)
E{1}=1.0;
Em=Input_Image;
...
[uu,vv]=find(Overlapping_Mat==min(min(Overlapping_Mat)));
MM_UV=max(size(uu));
...
[uuu,vvv]=find(E{i}(s+uu-1:s+uu,t+vv-1:t+vv)==min(min(E{i}(s+uu-1:s+uu,t+vv-1:t+vv))));
MM_UV=max(size(uuu));
Rand_Number_U=MM_UV*rand();
Rand_Number_V=MM_UV*rand();
